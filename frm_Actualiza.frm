...
Private Sub lbl_Actualizar_Click()
    ...
    Me.Repaint
    DoEvents
' === Flex Plan - Selecci�n de hoja solamente ===
' === Flex Plan - Selecci�n de hoja solamente ===
...
If Me.chk_Ordenes.Value Then
    ...
    If Dir(rutaCompleta) <> "" Then
        ...
        ' ? Fecha v�lida, continuar con carga
...
End If
        ...
        Else
            MsgBox "La tabla de �rdenes no tiene datos suficientes.", vbExclamation
        End If
    ...
    Else
        MsgBox "No se encontr� el archivo de �rdenes.", vbInformation
   ...
   If Me.chk_InvLocWIP.Value Then
        ...
        If Dir(rutaArchivos & vArchivo) <> "" Then
           ...
           '     Inv Location    Box Unit    Part#   Inj.Date Min    Dept    Type    Flag Ord
            ...
            vLstRen = Range("A65536").End(xlUp).Row
            ' === OPTIMIZACION: FillDown m�s r�pido que AutoFill ===
            If vLstRen > 2 Then
                Range("H2:I2").Copy
...
If Me.chk_LoadFactor.Value Then
    ...
    ' �ltima fila con datos
    vLstRen = Range("A65536").End(xlUp).Row

    ' === OPTIMIZACION: FillDown es m�s r�pido que AutoFill ===
    If vLstRen > 2 Then
        Range("M2:N2").Copy
...
If Me.chk_Capacidades.Value Then
    ...
    ' === 1) Tomar el libro ACTIVO (no el que contiene el c�digo) ===
    If Application.Workbooks.Count = 0 Then
        MsgBox "No hay libros abiertos.", vbExclamation
        Exit Sub
    End If
    Set wb = Application.ActiveWorkbook   ' <- clave para tu s�ntoma
    vPlan = wb.Name

    ' === 2) Asegurar que la estructura del libro permita agregar hojas ===
    If wb.ProtectStructure Then
        MsgBox "El libro est� protegido (estructura). Desprot�gelo para crear hojas.", vbExclamation
        Exit Sub
    End If
...
End If
    ...
    ' === OPTIMIZACION: Restaurar configuraci�n al FINAL ===
...
MDE:
    ' === OPTIMIZACION: Restaurar configuraci�n en caso de error ===
    On Error Resume Next
    Application.Calculation = prevCalc
    Application.ScreenUpdating = prevScreen
    Application.EnableEvents = prevEvents
...
Private Function GetOrCreateSheet(ByVal wb As Workbook, ByVal baseName As String) As Worksheet
    ...
    If ws Is Nothing Then
        ...
        If Err.Number <> 0 Then
            ...
            MsgBox "Ya exist�a '" & baseName & "'. Se cre� '" & ws.Name & "'.", vbInformation
        End If
        On Error GoTo 0

        MsgBox "Se cre� la hoja '" & ws.Name & "'.", vbInformation
    End If
...
Private Sub ForzarFechaEnColumna(ws As Worksheet, ByVal col As String, ByVal lastRow As Long)
    ...
    For Each c In r.Cells
        ...
        ' �ltimo intento: conversi�n gen�rica
        On Error Resume Next
